...
- A `metronome` source generated from Slopsmith's beat grid and shared metronome volume setting
...
The Slopsmith metronome source defaults to muted, uses the same volume setting as Slopsmith's bundled metronome controls, and emits a two-channel signal before channel routing. Route it to a playback pair and click **Apply Routing** to hear click pulses on song beats.
...
- `screen.js` enumerates outputs, detects active stems, and applies channel-pair routing.
